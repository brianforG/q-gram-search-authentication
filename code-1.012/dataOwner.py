...
class dataOwner(object):
    def __init__(self, D, q, optimizer="gs_square",length_filtering=False,representative_grams=False,empty_set_authentication=False,compress=False,compressor="lzma"):
        ...
        #print("DO init time: %.3fs"%(time_init2-time_init1))
        #print("--------------------DO init end--------------------------")
    ...
    def construct(self):
        ...
        message=self.mht1.get_merkle_root()
        message_hash = SHA256.new()
        message_hash.update(message.encode())
        self.S_dic=self.signer.sign(message_hash)
        ...
        message=self.mht2.get_merkle_root()
        message_hash = SHA256.new()
        message_hash.update(message.encode())
        self.S_inv=self.signer.sign(message_hash)
